    ...
    try {
      const response = await fetch("http://localhost:3333/api/articles", {
        ...
        body: JSON.stringify({
          ...
          resumo,     
          categoria, 
          tags        // O array de strings puro, o backend converte pra JSON
        }),
      });
      ...
      if (response.ok) {
        ...
        setErro(data.message || "Erro ao publicar artigo.");
      }
    } catch (error) {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#0B0E13] text-[#F9FAFB] font-sans">
      ...
      <main className="flex flex-col items-center py-12 px-4 flex-grow w-full">
        ...
        {/* Cabeçalho da Página (Voltar + Título) */}
        <div className="w-full max-w-[640px] flex flex-col gap-10">
          ...
          <div>
            <h1 className="text-[36px] font-medium text-white leading-none">Criar Novo Artigo</h1>
            <p className="text-sm text-[#9DA6AF] mt-2 font-light">Compartilhe seu conhecimento com a comunidade</p>
          </div>

          {/* Container do Formulário */}
          <form onSubmit={handlePublicar} className="w-full border border-[#2B303B] bg-[#0B0E13] p-9 flex flex-col gap-6">
            ...
            <div className="flex flex-col sm:flex-row gap-3 w-full mt-4">
              <button
                type="submit"
                disabled={loading}
                className="h-[36px] flex-grow bg-[#07B6D5] text-[#0B0E13] text-sm font-medium flex justify-center items-center hover:bg-opacity-90 transition-all disabled:opacity-50"
              >
